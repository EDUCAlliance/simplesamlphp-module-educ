...
namespace SimpleSAML\Module\educ\MetadataStore;

/**
 * educ:MDQ metadatastore class
 *
 * Original MDQ module modified:
 * - Returns null if the entity is not found, instead of throwing an exception. Logs as error.
 *
 ...
 * 		'cachedir' => $runtime . '/mdx-cache', // optional, recommended
 * 		'cachelength' => 86400, //opcionális,
 * 		'validateFingerprint' => '91:81:AD:2B:F1:C1:4E:47:93:A2:9D:49:34:B7:77:62:4F:2F:98:43' // optional, recommended
 * 	],
 * ```
 */
 ...
 * @author Uherkovich Péter, PTE
 * @package SimpleSAMLphp
 */

class MDQ extends \SimpleSAML\Metadata\Sources\MDQ //MetaDataStorageSource
{
    /**
     * The URL of the MDQ server (url:port)
     *
     * @var string
     */
     ...
     * @var string|null
     */
    private ?string $validateCertificate = null;
    ...
    {
        MetaDataStorageSource::__construct();
	 ...
	 * @throws Error\ConfigurationError
	 */
     ...
     * @throws Exception If an error occurs while loading metadata from the cache.
     */
    private function getFromCache(string $set, string $entityId): ?array
    {
        ...
        /* Ensure that this metadata isn't older than the cachelength option allows. This
    ...
    public function getMetaData(string $index, string $set): ?array
    {
        ...
        try {
            ...
            Logger::error(__CLASS__ . ': error downloading metadata: ' . $e->getMessage());
            $xmldata = false;
        }
		...
		try {
			...
			Logger::info('Exception occurred while fetching metadata for "' . $index . '" from ' . $mdq_url . ': '. $e->getMessage().' at '.$e->getFile().' #'.$e->getLine());
			Logger::debug(substr($xmldata, 0, 512));
			return null;
		}
